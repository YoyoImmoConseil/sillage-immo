...
import {
  ...
  registerUploadedClientPresentedDocument,
  resolvePresentedDocumentUploaders,
  uploadClientPresentedDocument,
...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const documents = await listPresentedDocumentsForClient(
      presentedId,
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const document = await registerUploadedClientPresentedDocument({
    ...
    if (file.size > PRESENTED_DOCUMENT_MAX_BYTES) {
      return NextResponse.json({ ok: false, message: "Fichier supérieur à 25 Mo." }, { status: 413 });
    }
    ...
    const document = await uploadClientPresentedDocument({
      presentedPropertyId: presentedId,
      clientProfileId: session.clientProfile.id,
      file,
      label,
