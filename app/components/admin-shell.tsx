        ...
        <div className="w-full space-y-4 px-6 py-8 md:px-10 xl:px-14 2xl:px-20">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.18em] text-sand/70">Back-office Sillage Immo</p>
              <h1 className="text-3xl font-semibold">{title}</h1>
              {description ? <p className="max-w-3xl text-sm text-sand/78">{description}</p> : null}
            </div>
            <div className="space-y-2 text-right">
              <p className="text-sm text-sand/78">
          ...
          <nav className="flex flex-wrap gap-2">
            {visibleItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded border border-sand/30 px-3 py-2 text-sm text-sand/88 transition hover:bg-sand/10"
              >
                {item.label}
              </Link>
            ))}
