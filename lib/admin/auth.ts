...
export const getAdminPageContext = async (): Promise<AdminContext | null> => {
  ...
  fetch("http://127.0.0.1:7695/ingest/34db18ce-fe4a-4a99-91a2-c9c0aaded505", {
    ...
    body: JSON.stringify({
      sessionId: "cada68",
      runId: requestHeaders.get("x-vercel-id") ?? `admin-page-${Date.now()}`,
      hypothesisId: "H2",
      location: "lib/admin/auth.ts:getAdminPageContext",
      message: "Resolving admin page context",
      data: {
        hasDedicatedAdminCookie: Boolean(accessToken),
      },
      timestamp: Date.now(),
    }),
  }).catch(() => {});
    ...
    if (user) {
      ...
      fetch("http://127.0.0.1:7695/ingest/34db18ce-fe4a-4a99-91a2-c9c0aaded505", {
        ...
        body: JSON.stringify({
          ...
          hypothesisId: "H4",
          location: "lib/admin/auth.ts:getAdminPageContext",
          message: "Dedicated admin cookie validated to a Supabase user",
          data: {
            hasUser: true,
          },
          timestamp: Date.now(),
